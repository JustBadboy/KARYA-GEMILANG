...
function page(p){const dk=dl(p),P=D.pages,ch=P.filter(c=>c.parent==p.id&&c.show),tr=[];
  ...
  return `<section class="wrap">${tr.length?`<p class="mut">${tr.map(q=>q.group?E(q.title):`<a href="#/${q.id}">${E(q.title)}</a>`).join(' › ')} › ${E(p.title)}</p>`:''}<h1>${E(p.title)}</h1>${p.img?`<img class="${dk.length?'docsample':'pimg'}" src="${p.img}" alt="">`:''}<div class="prose">${fmt(p.body)}</div>${ch.length?`<div class="cards">${ch.map(c=>`<a class="card svc" href="#/${c.id}">${c.img?`<img src="${c.img}" alt="">`:''}<h3>${E(c.title)}</h3><p>${esc(ex(c.body))}</p></a>`).join('')}</div>`:''}${tail}</section>`}
...
const repKey=()=>{try{return localStorage.getItem('kgk_repkey')||''}catch(e){return ''}};
const repUrl=()=>String(D.site.repUrl||'').replace(/\/$/,'');
function ensureXLSX(){if(window.XLSX)return Promise.resolve();return new Promise((ok,no)=>{const s=document.createElement('script');s.src='https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js';s.onload=ok;s.onerror=()=>no(new Error('Tidak bisa memuat pustaka Excel (perlu internet).'));document.head.append(s)})}
const rfind=(s,re)=>{const f=s.fields.find(x=>re.test(x.label));return f?f.value:''};
async function repTest(){const m=$('#msg'),u=repUrl(),k=$('#rk').value.trim();
  if(!u){m.textContent='Isi Alamat Worker dulu.';return}
  try{localStorage.setItem('kgk_repkey',k)}catch(e){}
  m.textContent='Menguji koneksi…';
  try{const r=await fetch(u+'/list',{headers:{Authorization:'Bearer '+k}});
    m.textContent=r.ok?'Terhubung.':r.status==401?'Kunci Admin salah.':'Gagal: server merespons '+r.status}
  catch(e){m.textContent='Gagal terhubung: '+e.message}}
async function repLoad(){const m=$('#msg'),u=repUrl(),k=repKey();
  if(!u||!k){m.textContent='Isi Alamat Worker dan Kunci Admin dulu.';return}
  m.textContent='Memuat laporan…';
  try{const r=await fetch(u+'/list',{headers:{Authorization:'Bearer '+k}});
    if(!r.ok)throw new Error('server merespons '+r.status);
    REP=(await r.json()).items||[];m.textContent=REP.length?`${REP.length} pengajuan dimuat.`:'Belum ada pengajuan masuk.';draw()}
  catch(e){m.textContent='Gagal memuat: '+e.message}}
async function repDel(id){const u=repUrl(),k=repKey(),m=$('#msg');
  try{await fetch(u+'/delete/'+encodeURIComponent(id),{method:'DELETE',headers:{Authorization:'Bearer '+k}});REP=REP.filter(x=>x.id!=id);draw()}
  catch(e){m.textContent='Gagal menghapus: '+e.message}}
async function repXLAll(){const m=$('#msg');
  if(!REP.length){m.textContent='Muat laporan dulu sebelum mengunduh.';return}
  m.textContent='Menyiapkan Excel…';
  try{await ensureXLSX();
    const rows=[['Tanggal','Layanan','Nama','Kontak','Detail Isian']];
    REP.forEach(s=>rows.push([new Date(s.ts).toLocaleString('id-ID'),s.service,rfind(s,/nama/i),rfind(s,/email|telepon|whatsapp/i),s.fields.map(f=>f.label+': '+f.value).join(' | ')]));
    const ws=XLSX.utils.aoa_to_sheet(rows);ws['!cols']=[{wch:18},{wch:26},{wch:20},{wch:22},{wch:70}];
    const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'Laporan');
    XLSX.writeFile(wb,'laporan-pengajuan.xlsx');m.textContent='Excel laporan diunduh.'}
  catch(e){m.textContent='Gagal membuat Excel: '+e.message}}
async function repInv(id){const s=REP.find(x=>x.id==id);if(!s)return;const m=$('#msg');m.textContent='Menyiapkan invoice…';
  try{await ensureXLSX();
    const rows=[[D.site.name],['INVOICE'],[],['Kepada',rfind(s,/nama/i)],['Kontak',rfind(s,/email|telepon|whatsapp/i)],['Tanggal',new Date(s.ts).toLocaleDateString('id-ID')],['No. Pengajuan',s.id],[],['Layanan','Jumlah','Harga Satuan','Total'],[s.service,1,0,0],['','','Total',0]];
    const ws=XLSX.utils.aoa_to_sheet(rows);
    ws['D10']={t:'n',v:0,f:'B10*C10'};ws['D11']={t:'n',v:0,f:'D10'};
    ws['!cols']=[{wch:30},{wch:10},{wch:16},{wch:16}];
    const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'Invoice');
    XLSX.writeFile(wb,'invoice-'+s.id+'.xlsx');m.textContent='Invoice diunduh. Isi Harga Satuan langsung di Excel, Total terhitung otomatis.'}
  catch(e){m.textContent='Gagal membuat invoice: '+e.message}}
...
function draw(){const y=scrollY,s=D.site,f=(k,l)=>`<label>${l}<input data-s="${k}" value="${esc(s[k])}"></label>`;
  ...
  if(tab=='lap'){b=`<div class="card"><h3>Sambungan Laporan</h3><p class="mut">Alamat Worker tersimpan di website dan dipakai formulir untuk mengirim data pengajuan. Kunci Admin hanya tersimpan di peramban ini dan dipakai untuk membaca laporan; jangan dibagikan.</p><div class="grid">${f('repUrl','Alamat Worker (URL)')}<label>Kunci Admin<input id="rk" type="password" value="${esc(repKey())}" autocomplete="off"></label></div><p><button class="pri" data-a="repsave">Simpan &amp; Tes Koneksi</button> <button data-a="repload">Muat Laporan</button> <button data-a="repxl">Unduh Semua (Excel)</button></p></div>
${REP.length?REP.map(s=>`<div class="card row"><div style="flex:1"><b>${esc(s.service)}</b><br><span class="mut">${esc(new Date(s.ts).toLocaleString('id-ID'))} · ${esc(rfind(s,/nama/i)||'-')} · ${esc(rfind(s,/email|telepon|whatsapp/i)||'-')}</span></div><button data-a="inv" data-id="${esc(s.id)}">Invoice (Excel)</button><button data-a="repdel" data-id="${esc(s.id)}">Hapus</button></div>`).join(''):'<p class="mut">Belum ada laporan dimuat. Isi pengaturan di atas, lalu klik Muat Laporan.</p>'}`}
...
function sendMail(b){const box=b.closest('[data-v]'),v=D.pages.find(x=>x.id==box.dataset.v),m=$('#fm'),to=D.site.email,F=fields(v),g=f=>((box.querySelector('#f_'+f.id)||{}).value||'').trim();
  ...
  if(repUrl())fetch(repUrl()+'/submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({service:v?(v.at||v.title):(sv?g(sv):'Konsultasi Umum'),fields:F.map(f=>({label:f.label,value:g(f)}))})}).catch(()=>{});
...
document.addEventListener('click',async e=>{
  ...
  else if(a=='repsave')repTest();
  else if(a=='repload')repLoad();
  else if(a=='repxl')repXLAll();
  else if(a=='inv')repInv(d.id);
  else if(a=='repdel')repDel(d.id);
  else if(a=='save')save();
});
let tp=[];
